...
def parse_args():
    ...
    parser.add_argument("--eval_every_n_steps", type=int, default=1000, help="Evaluate every n steps.")
    parser.add_argument("--gen_images_every_n_steps", type=int, default=1000, help="Generate images every n steps and calculate FID.")
    parser.add_argument("--generate_n_images_eval", type=int, default=10, help="Number of images generated during eval.")
    parser.add_argument(
        "--gdown_link_real_data",
        type=str,
        default=None,
        required=False,
        help="Google Drive download link.",
    )
    parser.add_argument("--train_text_encoder", action="store_true", help="Whether to train the text encoder")
    parser.add_argument("--custom_text_encoder", type=str, default=None, help="Path to custom text encoder")
...
def download_eval_data(args):
    import gdown
    import subprocess
    url = f'https://drive.google.com/uc?id={args.gdown_link_real_data}'
    filename = "real_data"
    output = f'./{filename}.zip'
    print(output)
    gdown.download(url, output, quiet=False)
    print(f'unzip -o {output}')
    subprocess.call(f'unzip -o {output}'.split())
...
def main():
    ...
    if args.custom_text_encoder is not None:
        text_encoder = CLIPTextModel.from_pretrained(args.custom_text_encoder)
    else:
    ...
    # save the val dataset as image file for fid calculation
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step % args.eval_every_n_steps == 0 or global_step % args.gen_images_every_n_steps == 0 or global_step == 1:
                ...
                if eval_this_step:
                        ...
                        with torch.no_grad():
                            for step, batch in tqdm(enumerate(val_dataloader)):
                                ...
                                with accelerator.accumulate(unet):
                                    ...
                                    avg_loss = accelerator.gather(loss.repeat(args.train_batch_size)).mean()
                                    train_loss += avg_loss.item() / args.gradient_accumulation_steps
                                    loss_avg_eval.update(loss.detach_(), bsz)
                            if accelerator.is_main_process:        
                    ...
                    if global_step % args.gen_images_every_n_steps == 0:
                        ...
                        if accelerator.is_main_process:
                            ...
                            # metrics = evaluate_using_filepaths(
                            #                     synth_data="./synth_data/",
                            #                     real_data="./real_data/",
                            #                     classes_to_eval=["NORMAL", "DME", "CNV", "DRUSEN"]
                            #                     )
                            ...
                            #         "FID": metrics['FID'],
                            #         # "SSIM-Loss": eval_.iloc[0]['SSIM-Loss'],
                            #         # "MS-SSIM-Loss": eval_.iloc[0]['MS-SSIM-Loss'],
                            #         },
                            #         step=global_step)
                            
                            # ## EVAL 
                            # # eval_ = evaluate_datasets(
                            # # datasets=datasets,
                            # # sd_settings=sd_settings,
                            # # real_data="./real_data/",
                            # # inception_pretrained=True)

                            # # wandb.log({
                            # #         "Trained-Precision": eval_.iloc[0]['Precision'],
                            # #         "Trained-Recall": eval_.iloc[0]['Recall'],
                            # #         "Trained-Density": eval_.iloc[0]['Density'],
                            # #         "Trained-Coverage": eval_.iloc[0]['Coverage'],
                            # #         "Trained-FID": eval_.iloc[0]['FID'],
                            # #         },
                            # #         step=global_step)

                                                # move all components back to gpu
                            ...
                            for cl in classes:
                                img_list = list()
                                for i in range(num_images_to_wandb):
                                    img_list.append(wandb.Image(f"./synth_data/{cl}/{cl}-({str(i).zfill(len(str(100)))})-state0.png"))
                                    wandb.log({f"{cl}@{global_step}": img_list})
                    eval_this_step = False
                else:
                    gradient_accumulation_steps_for_eval += 1

                if gradient_accumulation_steps_for_eval == args.gradient_accumulation_steps - 1:
                    eval_this_step=True
                    gradient_accumulation_steps_for_eval = 0

...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--val_batch_size", type=int, default=4, help="Batch size (per device) for the val dataloader."
    )
    parser.add_argument(
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index, args=parse_args()):
        ...
        if self.image_captions_filename:
            ...
            # for i in range(len(self.instance_prompt)):
            #     if self.instance_prompt[i] in pt:
            #         pt=self.instance_prompt[i]
...
def main(args):
    ...
    if len(files) < len(train_image_dataset):
        for i in tqdm(range(len(train_image_dataset))):
            ...
            img.save(f"./train_images/{train_image_dataset[i][args.caption_column]}-({i}).jpeg")
    else:
        print("Found dataset")
